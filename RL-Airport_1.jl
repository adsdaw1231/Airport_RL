...
function step!(env::AirportEnv, action)
    ...
    overtime_penalty = -25 * overtime_count
...
function QAgent(n_states, n_actions;
    α = 0.1,
    γ = 0.999,
    ϵ = 0.1)
    ...
    return QAgent(Q, α, γ, ϵ, n_actions)
end

function choose_action(agent::QAgent, s)

    if rand() < agent.ϵ
        return rand(1:agent.n_actions)
    else
        return argmax(agent.Q[s, :])
    end

end

function update_Q!(agent::QAgent, s, a, r, s2)

    best_next = maximum(agent.Q[s2, :])

    agent.Q[s, a] += agent.α * (
        r + agent.γ * best_next - agent.Q[s, a]
    )
...
function train_agent(P::Parameters, episodes::Int)
    ...
    n_states = get_total_states(P; n_bins=5) #状态空间
    n_actions = 3

    agent = QAgent(n_states, n_actions)
    ...
    for ep in 1:episodes
        ...
        while !is_terminated(env)
            ...
            # if r < -100
            #     S = env.S
            #     println("---- Abnormal reward detected ----")
            #     println("Episode: ", ep)
            #     println("Time: ", S.time)
            #     println("Reward: ", r)
            #     println("Action: ", a)
            ...
            #     println("Primary queue: ", length(S.primary_queue))
            #     println("Secondary queues: ", length.(S.secondary_queues))
            #     println("Open counters: ", S.n_open_counters)
            #     println("Busy counters: ", count(!isnothing, S.in_service))
            #     println("----------------------------------")
            # end
            # ====================

            update_Q!(agent, s, a, r, s2)

            s = s2
        end

        agent.ϵ = max(agent.ϵ * 0.999, 0.0001)

        # 记录本次 episode 总奖励
        push!(rewards, total_reward)

        if ep % 1000 == 0
            println("Episode ", ep, "total_reward", total_reward)
        end
...
function agent_decision!(S::State, P::Parameters, R::RandomNGs, agent::QAgent)
    # 编码当前状态
    s = encode_state_auto(S, P; n_bins=5)
    # 选择动作（贪婪）
    a = argmax(agent.Q[s, :])
    # 调整柜台数
    if a == 1  # 减少
...
function evaluate_agent(P::Parameters, agent::QAgent)
...
function evaluate_and_record(P::Parameters, agent::QAgent)
    # 创建输出目录和文件
    dir = pwd() * "/data_RL/seed" * string(P.seed)
    mkpath(dir)
