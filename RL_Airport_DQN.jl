...
function step!(env::AirportEnv, action)
    ...
    overtime_penalty = -5 * overtime_count
...
function DQNAgent(input_dim, n_actions; ϵ=0.1, γ=0.99)
    ...
    return DQNAgent(model, target_model, opt, ϵ, γ, n_actions)
end
...
function choose_action(agent::DQNAgent, S, P)

    if rand() < agent.ϵ
        return rand(1:agent.n_actions)
    end

    s = encode_state_vector(S, P)
    q = agent.model(s)
...
function soft_update(target, source; τ=0.005)
    for (p_target, p_source) in zip(Flux.params(target), Flux.params(source))
        p_target .= (1 - τ) .* p_target .+ τ .* p_source
    end
end

function train_dqn!(agent::DQNAgent, buffer::ReplayBuffer, batch_size=64)
    n = length(buffer.a)
    n < batch_size && return
    ...
    for i in idx
        ...
        q_vals = agent.model(s)
        q = q_vals[a]

        # target Q
        q_next = maximum(agent.target_model(s2))
        target = r + agent.γ * q_next

        loss += (q - target)^2
    end

    loss /= batch_size
    Flux.train!(() -> loss, Flux.params(agent.model), [], agent.optimizer)
    soft_update(agent.target_model, agent.model)
    ...
    for ep in 1:episodes
        ...
        while !is_terminated(env)
            ...
            s2_vec = encode_state_vector(env.S, P)
                ...
                push!(buffer.a, a)
                push!(buffer.r, r)
                push!(buffer.s2, s2_vec)

            # 每步训练（比 XGB 强很多）
            train_dqn!(agent, buffer, 64)
        end

        agent.ϵ = max(agent.ϵ * 0.995, 0.01)

        push!(rewards, total_reward)

        if ep % 10 == 0
            println("Episode ", ep, " Reward = ", total_reward, " ε=", agent.ϵ)
        end
    end
...
function get_action(agent::DQNAgent, S::State, P::Parameters)
...
function evaluate_and_record(P::Parameters, agent)
    # 创建输出目录和文件
    dir = pwd() * "/data_RL_DQN/seed" * string(P.seed)
    mkpath(dir)
